...

{
    if (ui->connect_butt->text() == "Connect")
    {
        ...
        if (!serial.isOpen())
        {
            ui->message_txt->setTextColor(Qt::red);
...
//    uartThread.transmit(tmp);
    ...
    ui->message_txt->setTextColor(Qt::darkGreen);
...
void MainWindow::rxMessage()
{
    ...
    if (index != -1)
    {
        ui->message_txt->setTextColor(Qt::blue);
        ui->message_txt->append(dataRX_str);
        ui->message_txt->setTextColor(Qt::black);
    }
    plotResponse();
}
